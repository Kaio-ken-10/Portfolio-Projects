...
-- Probability of death if contracted virus
...
SELECT date, sum(new_cases) as totalcases, sum(cast(new_deaths as int)) as totaldeaths, sum(cast(new_deaths as int))/sum(new_cases)*100 as globaldeathpecentage
from CovidDeaths
--where location like 'India'
where continent is not null
group by date
...
SELECT sum(new_cases) as totalcases, sum(cast(new_deaths as int)) as totaldeaths, sum(cast(new_deaths as int))/sum(new_cases)*100 as globaldeathpecentage
from CovidDeaths
--where location like 'India'
where continent is not null
--group by date
...
Looking at tot population vs vaccination
